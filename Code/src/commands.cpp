...
static bool cmd_sms_time_get(char* sms_number){
    char msg[256];
    snprintf(msg, sizeof(msg), "Sms interval: %f s.", battery_time_get_s());
    printf("%s %s\n", __func__, msg);
    return sms_send(sms_number, msg);
}

static bool cmd_sms_time_set(char* sms_number, double number){
    char msg[256];
    snprintf(msg, sizeof(msg), "Sms interval set. Success: %d", battery_time_set_s((float) number));
    printf("%s %s\n", __func__, msg);
    return sms_send(sms_number, msg);
}
...
bool command_handle(char* cmd_str, char* sms_number){
    ...
    } else if(0 == strcmp(cmd_str, "Sms interval get")){ 
        res = cmd_sms_time_get(sms_number);
